...
class TIS:
    ...
    def createPipeline(self, video_path=None):
        ...
        if self.livedisplay is True:
            p += " ! tee name=t"
            p += " ! queue ! videoconvert ! ximagesink"
        ...
        try:
            ...
            print("Error while creating this pipeline pipeline: {e}".format(e=err))
            raise
    ...
    def on_sync_message(self, bus, message):
        ...
        if message_name == "prepare-window-handle":
            ...
            imagesink.set_window_handle(GUI.mwin_id[self.cam_index])
    ...
    def initVideoCapture(self, video_path):
        self.stopPipeline()
        self.createPipeline(video_path=video_path)
    ...
    def _setcaps(self):
        ...
        SinkFormats.toString(self.sinkformat), self.width, self.height, self.framerate,)
    ...
    def convert_sample_to_numpy(self):
        self.samplelocked = True
        buf = self.sample.get_buffer()
        caps = self.sample.get_caps()
        mem = buf.get_all_memory()
